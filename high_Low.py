...
#will store the current player deck
player_Deck= [None]*15
...
def gen_Player_Deck():
    i = 0
    while(i < player_Deck_Size):
        deck_choser = (random.randint(0,4))
        if(deck_choser == 1):
           # print("hearts")
            card_Choser = (random.randint(-1,12))
            player_Deck[i]=deck_Hearts[card_Choser]
            i = i + 1
        if(deck_choser == 2):
            #print("diamonds")
            card_Choser = (random.randint(-1,12))
            player_Deck[i]=deck_Diamonds[card_Choser]
            i = i + 1
        if(deck_choser == 3):
           # print("clubs")
            card_Choser = (random.randint(-1,12))
            player_Deck[i]=deck_Clubs[card_Choser]
            i = i + 1
        if(deck_choser == 4):
            #print("spades")
            card_Choser = (random.randint(-1,12))
